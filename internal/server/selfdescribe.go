...
const selfDescribeTemplate = `{
  ...
  "http_api": {
    ...
    "attachments": {
      ...
      "quota": "attachment storage quota is an admin-adjustable setting (currently 50MB per account across live files) — an upload past it answers 413 'storage quota exceeded'; check usage with GET /api/files/list and free space with DELETE /api/files/{id} (download links in earlier letters stop working)",
